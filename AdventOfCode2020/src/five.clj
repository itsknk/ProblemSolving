...
;; Part One
;; Instead of zones or groups, this airline uses binary space partitioning to seat people. A seat might be specified like FBFBBFFRLR, where F means "front", 
...
;; Each letter tells you which half of a region the given seat is in. Start with the whole list of rows; the first letter indicates whether 
;; the seat is in the front (0 through 63) or the back (64 through 127). The next letter indicates which half of that region the seat is in, 
...
(def input-data
  (str/split-lines (slurp "resources/inputfive.txt")))


(defn solve
  [input]
  (read-string (apply str "2r" (map {\F \0 \B \1 \L \0 \R \1} input))))


(defn solution-one
  [input-data]
  (apply max (map solve input-data)))


;; Part One -> (solution-one input-data) = 901
...
(defn solution-two
  [input-data]
  (let [taken (set (map solve input-data))
        seats (set (range (apply min taken) (inc (apply max taken))))]
    (first (set/difference seats taken))))


;; Part Two -> (solution-two input-data) = 661
